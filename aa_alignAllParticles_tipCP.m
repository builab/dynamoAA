...
pixelSize = 14.00;
boxSize = 60;
filamentListFile = 'manualFilamentCPList.csv'; % Use manualFilamentCPList.csv if manual fixed, otherwise, filamentCPList.csv
alnDir = sprintf('%sintraAln', prjPath);
particleDir = sprintf('%sparticles', prjPath);
mw = 12; % Number of parallel workers to run
gpu = [0 1]; % Alignment using gpu
template_name = 'average_sortCP.em'; % If you have a better reference, use it instead
tableFileName = 'merged_particles_tipCP.tbl'; % merged particles table all
tableOutFileName = 'merged_particles_tipCP_align.tbl'; % merged particles table all
starFileName = 'merged_particles_tipCP.star'; % star file name for merged particles
...
outputFile = 'average_allParticles_tipCP.em';
